...
import { selectDepartInfo } from '../../../redux/selectors';
import s from './QueueByDepartSection.module.css';

export const QueueByDepartSection = () => {
      ...
      <ul>
        {departmentInfo.map((el) => (
          <li
            ...
            <span className={`${s.elem} ${s.dynamicElem}`}>{el.department.name}</span>
            <span className={`${s.elem} ${s.dynamicElem}`}>{el.online.in_process}</span>
            <span className={`${s.elem} ${s.dynamicElem}`}>{el.online.waiting}</span>
            <span className={`${s.elem} ${s.dynamicElem}`}>
              {normalizeTime(el.online.max_waiting_time).string}
            </span>
            <span className={`${s.elem} ${s.dynamicElem}`}>{el.offline.waiting}</span>
            <span className={`${s.elem} ${s.dynamicElem}`}>
